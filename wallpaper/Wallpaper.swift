// Night Garden (real-3D scene) as a desktop wallpaper.
//
// A variant of Desktop Habitats' Wallpaper.swift (Chase Lean, MIT) that loads the Night
// Garden scene under its own name, URL scheme and bundle identifier, so it installs next
// to Desktop Habitats without touching it. Feed is gone: a garden has nothing to feed.
//
...
// the scene inside this app bundle, served over a private scheme so its module
// imports resolve the way they do from a web server.
//
// The window never takes mouse events. The pointer reaches the insects another way: the
...
let sceneScheme = "night-garden"
let sceneHost = "local"
let scenePage = "/scenes/night-garden/wallpaper.html"

/// Serves the bundled copy of the scene to the web view.
...
final class Reporter: NSObject, WKScriptMessageHandler {
  ...
  func userContentController(
    ...
    NSLog("night-garden page: \(message.body)")
  }
}
...
final class Wallpaper: NSObject, WKNavigationDelegate {
  ...
  init(screen: NSScreen, root: URL) {
    ...
    view.underPageBackgroundColor = NSColor(
      calibratedRed: 0.016, green: 0.043, blue: 0.035, alpha: 1)
    ...
    window.backgroundColor = NSColor(calibratedRed: 0.016, green: 0.043, blue: 0.035, alpha: 1)
  ...
  func webView(
    ...
    NSLog("night-garden: the scene did not load: \(error.localizedDescription)")
  }
    ...
    view.evaluateJavaScript(
      ...
      (() => {
        ...
        return JSON.stringify({
          ...
          covered: Boolean(document.querySelector('#loading')) && !document.querySelector('#loading').hidden,
      ...
      NSLog("night-garden page state: \(value ?? error?.localizedDescription ?? "unreadable")")
    }
  }
  ...
  func snapshot(to file: URL, then done: @escaping () -> Void) {
    view.takeSnapshot(with: nil) { image, _ in
      ...
      NSLog("night-garden: wrote \(file.path)")
    }
  }
}
...
final class Controller: NSObject, NSApplicationDelegate, NSMenuDelegate {
  ...
  /// The choice outlives a restart, so a paused garden is still paused after logging in.
  ...
  func applicationDidFinishLaunching(_ note: Notification) {
    ...
    // stopped under it, unless the garden has already been asked for deliberately.
    ...
    // `kill -USR1` writes what the first screen is showing to /tmp/night-garden.png.
  ...
  private func snapshot() {
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 4) { [weak self] in
      first.probe()
      first.snapshot(to: URL(fileURLWithPath: "/tmp/night-garden.png")) {
        self?.applyRate()
      }
    }
  }
  ...
  /// The agent's only visible piece: a moon in the menu bar that can stop the garden.
  private func addMenu() {
    let item = NSStatusBar.system.statusItem(withLength: NSStatusItem.squareLength)
    let symbol = NSImage(systemSymbolName: "moon.stars", accessibilityDescription: "Night Garden")
    symbol?.isTemplate = true
    item.button?.image = symbol
    if symbol == nil { item.button?.title = "Night Garden" }
    item.button?.toolTip = "Night Garden"
    ...
    if item.button?.window == nil || !item.isVisible {
      NSLog("night-garden: the menu bar item did not appear")
    }
  }
